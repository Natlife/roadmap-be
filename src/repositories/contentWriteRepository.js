  ...
  if (id != null) {
    try {
      ...
      await db.query(
        ...
        [id, topic.title, topic.description || '', topic.emoji || 'book', topic.levelLabel || 'Beginner', topic.estimatedHours || 4, toUpperEnum(topic.accessLevel, 'FREE')]
      );
    }
  ...
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO topics (title, description, emoji, level_label, estimated_hours, access_level, status)
       VALUES (?, ?, ?, ?, ?, ?, 1)`,
      [topic.title, topic.description || '', topic.emoji || 'book', topic.levelLabel || 'Beginner', topic.estimatedHours || 4, toUpperEnum(topic.accessLevel, 'FREE')]
    );
    newId = result.insertId;
  }
...
async function upsertLesson(lesson, db = pool) {
  ...
  if (id != null) {
    try {
      ...
      await db.query(
        `UPDATE lessons SET title = ?, summary = ?, order_index = ?, access_level = ?, estimated_minutes = ? WHERE id = ?`,
        [lesson.title, lesson.summary || '', lesson.orderIndex || 0, toUpperEnum(lesson.accessLevel, 'FREE'), lesson.estimatedMinutes || 15, id]
      );
    }
  ...
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO lessons (topic_id, title, summary, order_index, access_level, estimated_minutes, status)
       VALUES (?, ?, ?, ?, ?, ?, 1)`,
      [lesson.topicId, lesson.title, lesson.summary || '', lesson.orderIndex || 0, toUpperEnum(lesson.accessLevel, 'FREE'), lesson.estimatedMinutes || 15]
    );
    newId = result.insertId;
  }
      ...
      await db.query(
        `INSERT INTO steps
         ...
         ON DUPLICATE KEY UPDATE
           ...
           checklist_json = VALUES(checklist_json), pass_threshold = VALUES(pass_threshold),
           estimated_minutes = VALUES(estimated_minutes), xp_reward = VALUES(xp_reward)`,
        [id, step.lessonId, step.title, step.summary || '', step.orderIndex || 0, toUpperEnum(step.accessLevel, 'FREE'), step.note || '', step.theory || '', step.codeSnippet || '', step.codeLanguage || 'text', JSON.stringify(step.checklist || []), step.passThreshold || 80, step.estimatedMinutes || 10, step.xpReward || 20]
      );
    }
    ...
    const [result] = await db.query(
      `INSERT INTO steps
         (lesson_id, title, summary, order_index, access_level, note, theory,
          code_snippet, code_language, checklist_json, pass_threshold, estimated_minutes, xp_reward, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
      [step.lessonId, step.title, step.summary || '', step.orderIndex || 0, toUpperEnum(step.accessLevel, 'FREE'), step.note || '', step.theory || '', step.codeSnippet || '', step.codeLanguage || 'text', JSON.stringify(step.checklist || []), step.passThreshold || 80, step.estimatedMinutes || 10, step.xpReward || 20]
    );
    newId = result.insertId;
  }
...
async function replaceStepBlocks(stepId, blocks, db = pool) {
  ...
  for (let index = 0; index < blocks.length; index += 1) {
    ...
    const codeLanguage = block.codeLanguage || block.code_language || '';
    const itemsRaw = block.items || block.items_json || [];
    const itemsJson = typeof itemsRaw === 'string' ? itemsRaw : JSON.stringify(itemsRaw);
